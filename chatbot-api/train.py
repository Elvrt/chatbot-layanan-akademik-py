...
# --- TAMBAHAN UNTUK 10-FOLD CROSS VALIDATION & PLOTTING ---
...
def load_cached_data():
    ...
    try:
        if (
            ...
            print("⚠️ Model belum tersedia. Silakan retrain dulu di terminal.")
    except Exception as e:
...
# ==========================================================
# 🔹 4. FUNGSI PELATIHAN LOKAL (TERMINAL SAJA)
# ==========================================================
def latih_dari_file():
    global model, df_kategori, df_knowledge
    ...
    print(f"\n🔄 Membaca data langsung dari {DATASET_FILE}...")
    ...
    # ==========================================================
    # 🔹 PROSES EVALUASI (10-FOLD CROSS VALIDATION)
    # ==========================================================
    print("\n" + "="*50)
    print("📊 MENGHITUNG METRIK EVALUASI (10-Fold Cross Validation)")
    print("="*50)
    ...
    try:
        ...
        # Inisialisasi Stratified K-Fold (k=10)
        cv = StratifiedKFold(n_splits=10, shuffle=True, random_state=42)
        
        # Lakukan prediksi silang menggunakan seluruh data
        y_pred_cv = cross_val_predict(eval_model, X, Y, cv=cv)
        
        # Hitung & Tampilkan Akurasi
        akurasi = accuracy_score(Y, y_pred_cv)
        print(f"🎯 Rata-rata Akurasi (10 Folds): {akurasi * 100:.2f}%\n")
        
        print("📈 LAPORAN KLASIFIKASI (Gabungan 10 Folds):")
        print(classification_report(Y, y_pred_cv, zero_division=0))
        
        # --- MEMBUAT GAMBAR CONFUSION MATRIX ---
        print("🧮 Menyimpan gambar CONFUSION MATRIX...")
        cm = confusion_matrix(Y, y_pred_cv)
        
        # Ambil label unik untuk sumbu gambar
        labels = sorted(list(set(Y)))
        
        # Atur ukuran kanvas gambar
        ...
        plt.xlabel('Kategori Prediksi (Predicted)')
        
        # Simpan sebagai file PNG di dalam folder dataset
        cm_path = os.path.join(DATASET_DIR, 'confusion_matrix.png')
        plt.savefig(cm_path, bbox_inches='tight')
        plt.close() # Bersihkan memori kanvas
        
        print(f"✅ Gambar Confusion Matrix berhasil disimpan di: {cm_path}")
        
    except ValueError as ve:
        print(f"⚠️ Evaluasi 10-Fold gagal. Pastikan SETIAP kategori memiliki MINIMAL 10 data variasi pertanyaan. Detail Error: {ve}")
    except Exception as e:
        print(f"⚠️ Evaluasi gagal karena alasan lain: {e}")
    print("="*50 + "\n")

    # --- 3. TRAIN FINAL MODEL ---
    print("🔄 Training model final dengan 100% data...")
    ...
    # --- 5. LOAD ULANG ---
    load_cached_data()
    print("✅ Model berhasil dilatih secara lokal dan disimpan!")
...
# ==========================================================
# 🔹 7. RUN SERVER ATAU JALANKAN DARI TERMINAL
# ==========================================================
